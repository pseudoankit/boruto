package com.pseudoankit.boruto.presentation.ui.theme
...
val titleColor
    @Composable get() = if (!isSystemInDarkTheme()) DarkGray else LightGray

val descriptionColor
    @Composable get() = if (!isSystemInDarkTheme()) DarkGray.copy(alpha = 0.5f)
    else LightGray.copy(alpha = 0.5f)

val activeIndicatorColor
    @Composable get() = if (!isSystemInDarkTheme()) Purple500 else Purple700
...
val topAppBarContentColor: Color
    @Composable get() = if (!isSystemInDarkTheme()) Color.White else LightGray

val topAppBarBackgroundColor: Color
    @Composable get() = if (!isSystemInDarkTheme()) Purple500 else Color.Black